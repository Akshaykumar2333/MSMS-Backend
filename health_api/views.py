...
from .serializers import DescriptionSerializer, PrecautionSerializer, MedicationSerializer, DietSerializer, WorkoutSerializer
...
svc_file_path = os.path.join(BASE_DIR, 'health_api', 'models', 'svc.pkl')
...
def helper(disease):
    desc = Description.objects.filter(disease=disease).first()
    pre = Precaution.objects.filter(disease=disease).first()
    med = Medication.objects.filter(disease=disease).values_list('medication', flat=True)
    die = Diet.objects.filter(disease=disease).values_list('diet', flat=True)
    wrkout = Workout.objects.filter(disease=disease).values_list('workout', flat=True).first()

    return {
        'description': desc.description if desc else "",
        'precautions': [pre.precaution_1, pre.precaution_2, pre.precaution_3, pre.precaution_4] if pre else [],
        'medications': list(med),
        'diet': list(die),
        'workout': wrkout if wrkout else ""
    }
...
class PredictAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not symptoms:
            return Response({'message': 'Please provide symptoms.'}, status=400)

        user_symptoms = [symptom.strip() for symptom in symptoms]
        ...
        disease_details = helper(predicted_disease)
